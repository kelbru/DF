...
img$common_name <- recode(img$common_name, "Common Racoon" = "Common Raccoon")
...
range(img$timestamp)#check range

table(is.na(img$timestamp)) #check for NAs
...
largest distance is 114.920 m, minimum is 6.463m, average is 57.716m
...
table(unique(dep$placename)  %in% unique(img$placename)) #found an issue!

#lets see whats different
discrep <- mapply(setdiff, unique(dep$placename), unique(img$placename))
discrep #the missing one did not record any images
...
# Check for a `group_size` variable? (sum of indivudals that observer is sure are different)
...
##we are left with 448 independent observations
